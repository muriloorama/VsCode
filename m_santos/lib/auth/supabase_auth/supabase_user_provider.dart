...
class MSantosSupabaseUser extends BaseAuthUser {
  MSantosSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> mSantosSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = MSantosSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
